...
// @version      3.1.0
// @description  ChatGPTの回答終了を検知し、登録したプロンプトを順番に自動送信します。
...
(() => {
  ...
  const CONFIG = {
    ...
    checkInterval: 500,
    stableTime: 3000,
    minimumResponseTime: 1500,
    completionGraceTime: 1200,
    sendTimeout: 30000,
  ...
  const state = {
    ...
    lastGeneratingAt: 0,
    sentAt: 0,
    assistantCountBeforeSend: 0,
    assistantTextBeforeSend: '',
    lastAssistantText: '',
    lastTextChangedAt: 0,
    monitorTimer: null,
    nextSendTimer: null,
  ...
  const ChatGPTAdapter = {
    ...
    getSendButton() {
      ...
      return [...form.querySelectorAll('button')].find((button) => {
        ...
        const text = normalizeText(`${button.getAttribute('aria-label') || ''} ${button.textContent || ''}`);
        return /送信|send/i.test(text);
      }) || null;
    },
    ...
    getStopButton() {
      ...
      return [...document.querySelectorAll('button')].find((button) => {
        if (!isVisible(button)) return false;
        const text = normalizeText(`${button.getAttribute('aria-label') || ''} ${button.textContent || ''}`);
        return /生成を停止|応答を停止|stop generating|stop responding/i.test(text);
      }) || null;
    },

    isGenerating() {
      if (this.getStopButton()) return true;
      return [...document.querySelectorAll('[data-is-streaming="true"], [data-message-author-role="assistant"][data-is-streaming="true"]')].some((element) => isVisible(element));
    },
    ...
    isSendReady() {
      ...
      if (!this.isInputReady() || !button || !isVisible(button)) return false;
      return !button.disabled && button.getAttribute('aria-disabled') !== 'true' && !this.isGenerating();
    },

    getAssistantMessages() {
      const messages = [...document.querySelectorAll('[data-message-author-role="assistant"]')].filter((element) => isVisible(element));
      if (messages.length > 0) return messages;
      return [...document.querySelectorAll('article')].filter((article) => isVisible(article) && normalizeText(article.textContent).length > 0);
    },
    ...
    getLastAssistantText() {
      ...
      return normalizeText(lastMessage?.innerText || lastMessage?.textContent || '');
    },

    detectError() {
      const patterns = [/ネットワークエラー/i, /network error/i, /エラーが発生/i, /something went wrong/i, /問題が発生/i, /応答を生成できません/i, /failed to get response/i, /再生成/i, /regenerate/i, /再試行/i, /retry/i];
      const candidates = [...document.querySelectorAll('[role="alert"], [data-testid*="error"], main [class*="error"], main button')];
    ...
    async setPrompt(text) {
      ...
      await sleep(300);
      const enteredText = normalizeText(composer.value || composer.innerText || composer.textContent || '');
      if (!enteredText) throw new Error('入力処理後も入力欄が空です');
    },
  ...
  function isVisible(element) {
    ...
    return style.display !== 'none' && style.visibility !== 'hidden' && Number(style.opacity || 1) !== 0 && rectangle.width > 0 && rectangle.height > 0;
  }
  ...
  function getRandomWaitTime() {
    return Math.floor(CONFIG.minimumRandomWait + Math.random() * (CONFIG.maximumRandomWait - CONFIG.minimumRandomWait + 1));
  }

  function createDefaultPrompts() {
    return ['テスト1', 'テスト2', 'テスト3'];
  }
  ...
  function createStyles() {
    ...
    style.textContent = `
      #${CONFIG.panelId} { position: fixed; z-index: 2147483647; top: 70px; right: 18px; width: 430px; height: 430px; min-width: 340px; min-height: 250px; max-width: 95vw; max-height: 90vh; display: flex; flex-direction: column; overflow: hidden; resize: both; color: #eeeeee; background: rgba(25,25,28,.98); border: 1px solid #55555d; border-radius: 10px; box-shadow: 0 12px 35px rgba(0,0,0,.45); font-family: system-ui,-apple-system,"Segoe UI",sans-serif; font-size: 12px; }
  ...
  function createPanel() {
    ...
    panel.innerHTML = `
      <div class="runner-header"><span class="runner-title">ChatGPT 自動プロンプト</span><span class="runner-count">0 / 0</span><span class="runner-status">停止中</span></div>
      ...
      <div class="runner-list"></div><div class="runner-notice">準備完了</div>`;
  ...
  function bindPanelEvents(panel) {
    ...
    panel.addEventListener('input', (event) => {
      ...
      state.prompts[index] = event.target.value;
      updateScreen();
    });
  ...
  function enableDragging(panel, handle) {
    ...
    window.addEventListener('mousemove', (event) => {
      ...
      const left = Math.min(maximumLeft, Math.max(0, startLeft + event.clientX - startMouseX));
      const top = Math.min(maximumTop, Math.max(0, startTop + event.clientY - startMouseY));
  ...
  function savePanelPosition(panel) {
    const rectangle = panel.getBoundingClientRect();
    localStorage.setItem(CONFIG.positionStorageKey, JSON.stringify({ left: Math.round(rectangle.left), top: Math.round(rectangle.top), width: Math.round(rectangle.width), height: Math.round(rectangle.height) }));
  }
  ...
  function syncPromptsFromScreen() {
    ...
    if (inputs.length > 0) state.prompts = inputs.map((input) => input.value);
  }

  function addPrompt() {
    if (state.running) { showNotice('実行中は追加できません'); return; }
    syncPromptsFromScreen();
    if (state.prompts.length >= CONFIG.maximumPrompts) { showNotice('最大100件です'); return; }
    state.prompts.push('');
    renderPromptRows();
  }

  function deletePrompt(index) {
    if (state.running) { showNotice('実行中は削除できません'); return; }
    syncPromptsFromScreen();
    state.prompts.splice(index, 1);
    if (state.prompts.length === 0) state.prompts.push('');
    state.currentIndex = Math.min(state.currentIndex, state.prompts.length - 1);
    renderPromptRows();
  }

  function movePrompt(index, movement) {
    if (state.running) { showNotice('実行中は順番変更できません'); return; }
    syncPromptsFromScreen();
    const destination = index + movement;
    if (index < 0 || destination < 0 || destination >= state.prompts.length) return;
    const temporary = state.prompts[index];
    state.prompts[index] = state.prompts[destination];
    state.prompts[destination] = temporary;
    renderPromptRows();
  }

  function clearPrompts() {
    stopExecution('停止しました');
    state.prompts = createDefaultPrompts();
    state.currentIndex = 0;
    state.paused = false;
    renderPromptRows();
    showNotice('テスト1・2・3に戻しました');
  }

  function loadPromptsIntoScreen() {
    if (state.running) { showNotice('実行中は読込できません'); return; }
  ...
  function findNextPromptIndex(startIndex) {
    for (let index = Math.max(0, startIndex); index < state.prompts.length; index += 1) {
      if (normalizeText(state.prompts[index])) return index;
    }
    return -1;
  }
  ...
  function getProgress() {
    ...
    if (indexes.length === 0) return { current: 0, total: 0 };
    ...
    const active = indexes.includes(state.currentIndex) ? 1 : 0;
    return { current: Math.min(indexes.length, completed + active), total: indexes.length };
  }

  async function startExecution() {
    if (state.running) { showNotice('すでに実行中です'); return; }
    syncPromptsFromScreen();
    savePrompts();
    const firstIndex = findNextPromptIndex(0);
    if (firstIndex === -1) { setStatus('停止中'); showNotice('送信するプロンプトがありません'); return; }
    clearTimers();
  ...
  function stopExecution(message = '停止しました') {
    ...
    state.generatingObserved = false;
    state.lastGeneratingAt = 0;
    setStatus('停止中');
    showNotice(message);
  }
  ...
  async function resumeExecution() {
    ...
    if (nextIndex === -1) { showNotice('再開するプロンプトがありません'); return; }
    clearTimers();
  ...
  async function sendCurrentPrompt() {
    ...
    if (index === -1) { finishExecution(); return; }
    state.currentIndex = index;
    updateScreen();
    const existingError = ChatGPTAdapter.detectError();
    if (existingError) { stopBecauseOfError(existingError); return; }
    ...
    state.lastTextChangedAt = Date.now();
    state.responseStarted = false;
    state.generatingObserved = false;
    state.lastGeneratingAt = 0;
    ...
    try {
      ...
      await waitUntilInputReady(CONFIG.sendTimeout);
      if (!state.running) return;
      await ChatGPTAdapter.setPrompt(prompt);
      if (!state.running) return;
      await waitUntilSendReady(CONFIG.sendTimeout);
      if (!state.running) return;
      ...
      stopBecauseOfError(error?.message || String(error));
    }
  }

  async function waitUntilInputReady(timeout) {
    const startedAt = Date.now();
    while (Date.now() - startedAt < timeout) {
      if (!state.running) throw new Error('停止しました');
      if (ChatGPTAdapter.isInputReady()) return;
      await sleep(500);
    }
    throw new Error('入力欄が使用可能になりません');
  }

  async function waitUntilSendReady(timeout) {
    const startedAt = Date.now();
    while (Date.now() - startedAt < timeout) {
      if (!state.running) throw new Error('停止しました');
      if (ChatGPTAdapter.isSendReady()) return;
      await sleep(350);
    }
    throw new Error('送信ボタンが使用可能になりません');
  }

  function startResponseMonitoring() {
    if (state.monitorTimer) clearInterval(state.monitorTimer);
    state.monitorTimer = window.setInterval(checkResponseState, CONFIG.checkInterval);
  }

  function checkResponseState() {
    if (!state.running || state.waiting) return;
    const errorText = ChatGPTAdapter.detectError();
    if (errorText) { stopBecauseOfError(errorText); return; }
  ...
  function handleResponseFinished() {
    ...
    if (nextIndex === -1) { finishExecution(); return; }
    state.currentIndex = nextIndex;
    const waitTime = getRandomWaitTime();
    const waitSeconds = (waitTime / 1000).toFixed(1);
    setStatus('待機中');
    showNotice(`${waitSeconds}秒待機`);
    state.nextSendTimer = window.setTimeout(async () => {
      if (!state.running) return;
      state.waiting = false;
      setStatus('実行中');
      showNotice('次を送信します');
  ...
  function finishExecution() {
    ...
    state.lastGeneratingAt = 0;
    setStatus('完了');
    showNotice('すべて完了しました');
    playToneSequence([{ frequency: 660, duration: 140 }, { frequency: 880, duration: 160 }, { frequency: 1040, duration: 260 }]);
  }
  ...
  function stopBecauseOfError(message) {
    ...
    state.lastGeneratingAt = 0;
    setStatus('停止中');
    showNotice(`エラー: ${normalizeText(message).slice(0, 120)}`);
    playToneSequence([{ frequency: 320, duration: 180 }, { frequency: 220, duration: 280 }]);
  }
  ...
  function showNotice(message) {
    const notice = document.querySelector(`#${CONFIG.panelId} .runner-notice`);
  ...
  function playToneSequence(tones) {
    try {
      ...
      for (const tone of tones) {
        ...
        oscillator.stop(startTime + tone.duration / 1000 + 0.03);
